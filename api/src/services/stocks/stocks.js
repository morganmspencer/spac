...
function fmpApi(request) {
  ...
  return 'https://financialmodelingprep.com/api/v3' + request + `${keySep}apikey=${process.env.FMP_API}`
}
...
export const stockData = async({ symbol }) => {
  ...
  const fmpResponse = await fetch(
    fmpApi(`/profile/${spacReturn.symbol}`)
  )
  ...
  if ( spacReturn.ipoSymbol !== spacReturn.symbol ) {
    ...
    const fmpResponse = await fetch(
      fmpApi(`/profile/${spacReturn.ipoSymbol}`)
    )
